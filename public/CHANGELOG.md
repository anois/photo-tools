...
把"裁剪 / 旋转 / 边距"三件事合并成一个 **构图模式（Compose）**，作为 lookbar 第 6 个独立块（与 LOOK / 4 个 lookchip 并列）。设计语言：**暗房工作台 + 大画幅 ground glass** —— 暖深棕底 + 单一琥珀色 safelight，照片"悬浮"在画面中央，三种操作通过空间分层共存（照片内 = 裁剪、画框边缘 = 边距、顶部圆环 = 旋转）。

### ✨ 构图模式 · 全新外部层级功能

- **裁剪 / 旋转 / 四边距统一为一个直接拖拽界面**。打开后接管整个视口，所见即所得地操作渲染后的图像。
- **幽灵层**：拖拽裁剪与旋转时，画框之外的完整原图以 30% 透明度显示，让你看清"什么被裁掉了"。
- **每边可独立的边距**：`paddingTop` / `paddingRight` / `paddingBottom` / `paddingLeft` 加入 cfg + LOOK_KEYS，preset / share-code 自动覆盖（v:1 schema 保持向后兼容，老 preset 全部 null = 不影响）。拖拽边缘胶囊推条直接改 / 工作台数字输入直接编辑。
- **frame.minPadding 软提醒**：film-35 / film-mf / slide-mount / instax 各自声明了"画框正常运作所需的最小边距"。拖拽低于该阈值时画框对应边出现淡红警示带 + 工作台数字变红 —— **不阻止**，只提醒（"我知道我在做什么"的用户照样可以越界）。
- **任意角度旋转**：旋钮默认 90° 吸附，按住 `Shift` 进入自由旋转（步长 1°）。引擎本来就支持任意角度（`inscribedSafeArea`），这次只是把这能力暴露到 UI。
- **测量 HUD**：拖拽时跟随光标的小琥珀牌子实时显示当前数值 + 单位 + 推荐下限。
- **工作台数字可编辑**：底部三栏（CROP W/H、PAD T/R/B/L、ROT 度数）全部是可输入数字，对追求精确的用户友好。
