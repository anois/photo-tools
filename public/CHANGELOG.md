...
**🔦 关灯看样 · 减法** —— 按用户反馈：画在照片上的虚线框是噪音，不是信号。

### 🎨 工作台

- 移除拖动旋钮时画布上的琥珀虚线区域描边（「背景区」「阴影圈」等标注框）——对全画布类旋钮它圈住一切等于没说，对局部旋钮它恰好盖住你正在盯的位置；点按开关 / 分段按钮时的描边闪现一并移除
- 画布上方的参数读数 HUD 保留不变

## 1.16.2 · 2026-08-20

**🔦 关灯看样 · 更彻底** —— 按用户反馈加深：拖动旋钮时不该留下任何残影。

### 🎨 工作台

- 拖动任何工作台旋钮时，整个面板**完全隐去**（此前保留 5% 残影、面板底色和仪器卡面）——只留手指下那一行以半透明浮在画面上，透过它也能看到照片；桌面与移动端一致，图像变化零遮挡
- 试印条模式下顶栏（◫ 退出入口）同样改为半透明浮层
