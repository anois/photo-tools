...
Frames are organized into 5 visual families. The seg buttons in `#frame-seg` carry a `data-family` attribute so future CSS can group them visually.
...
| | `film-mf`   | solid `#e8d7ab` (aged amber fiber paper) | dark | `topPaddingBoost: 100, bottomPaddingBoost: 120` | 40 / 12 / 0.16 | aged gelatin silver print: deckle hairline + italic library notation + **vintage aging effects** baked into decorate — sepia multiply tint on the photo, diagonal screen-gradient partial fade (upper-left bleached, simulating decades of light exposure), radial corner vignette (handling oxidation), 18 deterministic foxing spots scattered across the paper margin (rejected from photo + caption zones; scale-free seed + per-candidate indexed hashing since 1.16.1, so the pattern is identical across drag/preview/export render scales). This is the print AFTER 50 years, not the fresh print |
| | `slide-mount` | solid `#e6dac0` (warm cream cardstock) | dark | `topPaddingBoost: 100, bottomPaddingBoost: 120` | 0 / 0 / 0 (photo is **inset**, not raised) | mounted-transparency look. Caption + topBadge text come from the standard caption + topTemplate systems, but slide-mount destructively paints the leather over them and **re-stamps them in decorate** with an embossed treatment (1.5·s px down-offset inverted-light layer underneath the dark text → "pressed into leather"). **Pebbled-leather surface** is a 128×128 `createPattern` tile (module-level cached, built once per context): cream base + ~180 discrete tile-wrapping pebble bumps each painted as a warm highlight ellipse offset upper-left + a sepia shadow ellipse offset lower-right (fixed lighting → 3D feel) + ±7 per-pixel grain on top. `pattern.setTransform(scale)` keeps pebble density physically constant across quality settings. Plus ~15 broad soft radial patches (low-frequency unevenness; scale-free seed + indexed hashing since 1.16.1). **Bevel cues** around the aperture: ~8 base-px highlight gradient on cream top+left + matching dark gradient on cream bottom+right. **Photo aperture**: heavy dark hairline (`rgba(15,8,4,0.82)`, ~1.4·outputPx) + 2-stop inset shadow inside the photo (24 base-px deep, top+left). **Outer wine border** `#3a1822` (28 base-px ring on all 4 sides) drawn last. **Engine contract**: `clientRender.js`/`worker.js` stash `args.captionImg` + `args.topBadgeImg` for any destructive decorate hook to re-stamp; non-destructive frames ignore them |
| **Print** | `halftone` | solid, color taken over by `params.halftone.paper` | dark | `radiusOverride: 0` | 0 / 0 / 0 (print is flat) | none — the frame's identity lives in its `fx` hook (two-ink halftone screening of the composed fg, see "Frame `fx` hook" below). `noCaption: true` — this frame renders NO EXIF caption (1.12.0 design ruling: the screened image is the whole print); caption/template knobs are inert while active, surfaced via the notation hint + template-compat hint. 7 cfg knobs (`htInk`/`htPaper` — first `color`-kind harness knobs with `swatches` array; `htCell`/`htPx`/`htAngle`/`htTone` sliders; `htShape` stepper). Paper takeover of bg.color follows the instax tintColors pattern |
...
**Frame `fx` hook (1.12.0+)** — the third frame hook, and the first that rewrites the photo's own pixels (`decorate` paints AROUND the photo, `clipPath` reshapes its outline). `frame.fx(ctx, layout, args)` runs AFTER the fg photo/collage cells are drawn and BEFORE caption / topTemplate badge / `decorate` / seal — so text and stamps stay crisp solid ink over the processed pixels. Mirrored across `clientRender.js` and `worker.js` like `decorate` (guarded `typeof frame.fx === 'function'` + try/catch — zero cost for frames without it). Hooks must be DOM-free: OffscreenCanvas (with a `document.createElement` fallback for old main-thread Safari) + ctx primitives. `halftone` is the reference implementation: it reads back the fg envelope region at a working resolution defined in base-1440 units (`workW = fgW_base / htPx` — **independent of render scale**; `chunk = px·scale` must NOT be floored at 1: the old `Math.max(1, …)` floor made workW scale-dependent whenever `px·scale < 1`, so the 0.2× lights-down drag frames screened at a 2.5× smaller lattice — ~40% of pixels flipped on release — and even the 0.5× preview disagreed with export for `htPx = 1`; fixed 1.16.1), runs one ImageData pass (Rec.709 luma → tone gamma → rotated-grid spot-function threshold → binary ink/paper), and blits back with `imageSmoothingEnabled = false` **when `chunk ≥ 1`** (chunky-dot identity) but **smoothing ON when `chunk < 1`** — nearest-neighbor *downscaling* a binary lattice below Nyquist shreds it into aliasing debris; smooth downscale shows the screen's true local average (the print seen from farther away), so release/settle only sharpens, never restructures. Scope is the fg envelope only (never the frame margin — a cream margin would otherwise sprout an ink speck at every screen-cell center); collage gutters inside the envelope are paper-colored and screen back to near-paper. Rotating the photo does NOT rotate the screen (fx works on composed canvas pixels, matching PS's flatten-then-filter behavior) — that's a feature. No fx result cache in v1: the working buffer is small (~0.5M px typical, one arithmetic pass) and a cache key would have to mix in crop/rotation/collage/all ht\* knobs; if Compose drags ever feel heavy, the P2 lever is doubling `htPx` while `isDragging`.
...
**Mobile** (≤700px): the lookbar grid is 4 rows (`44px 44px 44px 44px`) as of 1.1 — LOOK strip (row 1, full-width) / **Compose strip (row 2, full-width)** / [import + chips] (row 3) / [export + ZIP] (row 4). The +52px height (lookbar grew from 148px to 200px content + safe-area) is the cost for putting Compose at one thumb-tap, same precedent as LOOK earned its +44px in 0.18 for the same reason. Trajectory has been: 0.17 = 104px (2 rows), 0.18 = 148px (3 rows + LOOK), 1.1 = 200px (4 rows + Compose). Workshop stays exiled to a topbar pill — it's occasional deep-config, not per-photo.
...
- the corresponding bench number turns red
- the measurement HUD's aux row says "below min"

The warning **doesn't clamp** — the user can dial through anyway (the product direction is "more user-controllable knobs", not "guard rails everywhere"). Frames without `minPadding` declared (frosted-noir / gallery-white / torn) never warn — they have no decorations dependent on specific padding zones.
...
**Mobile parity (1.1.2+ → 1.2.0)**: the mobile Compose dialog is a **parallel UI**, not a CSS-reflow of the desktop bench. Same `<dialog id="compose-modal">` but new markup elements appear only on `max-width: 700px`, while the desktop `<footer class="compose-bench">` hides via `display: none` in the same media query. Both surfaces share the same JS state (`COMPOSE.cfg` / `setFocus` / `setRotationDeg`).

**1.2.0 — crop interaction rewrite ("Through the Aperture")**: the v1.1.x mobile crop kept the corner-bracket handle UX from desktop and the result was completely unusable on touch — `28×32px` handles smaller than a fingertip, finger occlusion of the very thing being grabbed, and "drag handle = resize / drag inside = pan" mode judging that requires mouse-style hover-test that touch cannot do. v1.2.0 throws all that out and adopts the iOS-native crop grammar (Apple Photos / Instagram / Lightroom Mobile):

- **Aperture is fixed; photo moves behind it.** A semi-opaque dark mask covers everything except a centered aperture rectangle. The user pans the photo with one finger and pinches with two — what's inside the aperture IS the crop. No handles to grab, no mode confusion.
- **Markup**: `<div id="compose-aperture-shell">` (mobile-only, gated by media query + `data-focus="crop"`). Inside it: `<canvas id="compose-aperture-photo">` (the rotated source pre-rendered once; CSS transform handles pan/pinch) + `<div id="compose-aperture-frame">` (the aperture border; its `box-shadow: 0 0 0 9999px rgba(8,6,4,0.78)` paints the dark mask, no extra DOM) + 4 corner brackets + rule-of-thirds grid + scale HUD.
- **Mask via box-shadow.** The "dark area outside the aperture" is one declaration — `box-shadow: 0 0 0 9999px rgba(8,6,4,0.78)` on the aperture frame. The frame itself is transparent; the huge spread paints everywhere except where the frame sits. Cheap and zero extra DOM.
- **Pre-rendered rotated bitmap.** On every entry to crop mode, `renderApertureBitmap()` draws the source bitmap with `cfg.rotation` baked in onto the aperture-photo canvas. From then on pan + pinch only update CSS custom props (`--ap-tx --ap-ty --ap-scale`) on that canvas — no per-frame canvas re-draw. GPU compositor handles the transform at 60fps.
- **Transform ↔ cfg.crop math** (in `app.js`'s `APERTURE` module). `applyApertureFromCfg()` reads cfg.crop and computes (`tx`, `ty`, `scale`) so the crop sub-rect of the photo lands exactly on the aperture rect. On `pointerup`, `writeCropFromTransform()` reverses it: cx = (ax-tx)/(s·bmW), cy = (ay-ty)/(s·bmH), cw = aw/(s·bmW), ch = ah/(s·bmH). Photo must always cover the aperture; `clampApertureTransform()` enforces this and auto-bumps `scale` if the user tries to drag past the edge.
- **Pinch math.** Standard "anchor under midpoint": at pinchstart record the photo-local point under the gesture midpoint; on every move, scale = scale0 · (dist/dist0), then tx/ty re-derived so the anchor stays under the current midpoint. Hard caps: down to `minScale` (photo just covers aperture), up to `minScale × 8`. Scale HUD on the right edge shows current zoom as a multiple of minScale.
- **`setPointerCapture` wrapped in try/catch.** Required for multi-pointer reliability — synthetic / non-trusted PointerEvents in some browsers throw on capture and would silently kill the pinch path. Lesson from MCP verification — the second `pointerdown` was silently failing because of an uncaught throw on `setPointerCapture`. Always wrap `setPointerCapture` calls when you have multi-touch handlers.
- **Aspect chips refactored as brass-style mini-plates** with shape glyphs encoding the aspect ratio (a small filled rectangle inside each chip with the actual pixel ratio of the aspect). On click → `applyAspectChipMobile(token)` re-fits cfg.crop to a max-area, centered rect of the new aspect, then re-calls `applyApertureFromCfg({ pulse: true })` which pulses the amber border for 600ms as a snap-acknowledgment.
- **Mobile chip handlers bind in capture phase** with `stopImmediatePropagation()` so the legacy desktop `bindAspectChips` doesn't also fire. Same pattern would apply for any future mobile-specific override on a button the desktop also binds.
- **Stage layering on mobile crop mode**: `compose-stage.aperture-active` class is set when aperture is on; it `visibility: hidden`s the regular canvas + handles + ghost. They stay in the DOM so a mode switch back can re-show them instantly — no remount.
- **Other modes unchanged**: pad still uses the 1.1.2 four-slider strip with L/R sync, rot still uses the slider bar. The aperture overlay is crop-mode-only.

Common mobile UX choices (apply across all three modes):

- **Mode switching** = floating right-side pill column on the photo, NOT a bottom tab bar (saves 60px vertical). 4 pills (crop / pad / rot / numeric ≡), 48×56 px each. Active pill is amber-filled.
- **Numeric editing** is collapsed by default. The ≡ pill opens a bottom-sliding sheet (`.compose-numeric-sheet`, `max-height: 45vh`) with the relevant inputs for the current mode. 95% of mobile users don't need it; for them the sheet stays hidden.
- **Padding mode uses 4 horizontal sliders + L/R sync toggle**. On-photo `.compose-pad-handle` is `opacity: 0 + pointer-events: none` under mobile. The sync toggle (default ON) mirrors L↔R drags so symmetric horizontal margins (the common case) are one slider gesture.
- **HUD bubble offsets away from finger** on mobile (touchX+30, touchY-60), with auto-flip when too close to viewport edges.

Touch-coarse rules (`@media (pointer: coarse)`) further enlarge crop corner / edge / pad / rot handles for any pointer:coarse device regardless of width — that block is independent of the mobile media query and applies to touch-capable laptops too.
...
`cfg.rotation` is degrees clockwise. Up through 1.0 the UI was constrained to 0 / 90 / 180 / 270 via two ↶↷ buttons in the workshop crop modal, but the engine accepts arbitrary angles. 1.1's Compose mode exposes this fully — default 90° snap with `Shift` for free rotation.
...
### Workshop design language — 「暗房纪律 darkroom discipline」 (1.12.1)

The workshop drawer's visual system, spec'd in `docs/workshop-darkroom-rev2.html`. One sentence: **amber = the user's hand** — machine defaults are neutral graphite; anything the user touched (active tool, modified-tool dots, handwritten EXIF overrides, dialed values) lights up the safelight amber (`#d4a574`, same lamp as Compose). Red (`--wk-danger`) is reserved for destructive actions only (remove seal, EXIF-stripped warn).

- **Tokens** live on `:root` with the `--wk-` prefix (`--wk-g0..g3` graphite steps, `--wk-line(-strong)`, `--wk-text/mute/dim`, `--wk-amber(-hi/-dim/-faint)`, `--wk-danger`) — declared at the top of the "DARKROOM INSTRUMENT CARDS" section in styles.css. Workshop rules should consume these, not hardcoded colors.
- **The 7 per-frame material card themes are retired** (mixing console / kraft / riso proof-press / archive paper / museum placard / brushed steel / wine leather — and the halftone riso card from 1.12.0). All frame cards share one graphite recipe; frame identity survives as a 2px left accent bar (`--fa` set per card class) + the mono tag line (old stamps/nameplates restyled, ornament sub-elements `display:none`d). Adding a frame's instrument card now means: reuse the unified classes' look, set the card's `--fa`, done — don't reintroduce a material theme (the drama budget belongs to the LOOK/main UI, not the drawer).
- **Typography**: Hanken (titles 600 / labels 400) + JetBrains Mono (eyebrows, readouts, tags — `tabular-nums`). Fraunces italic is banned inside the workshop. Sole material-font exception: Notation's handwriting column keeps Special Elite, colored amber — typewriter face + safelight both mean "a human wrote this".
- **Untouched readouts show `—`** (`frame.defaultReadout` in i18n.js is the em-dash in both locales). **P2 (pending)**: touched-state semantics — a central `setKnobReadout(el, valueText|null)` helper replacing the ~30 scattered `textContent` writes in sync/wiring, toggling an `.is-set` class (amber readout + amber fill track, dim `—` otherwise). Until then all readouts render amber uniformly.
- Frameless/`noCaption` inert zones dim via `.is-frameless` body class (45% + pointer-events off); the Caliper frameless toggle deliberately sits outside the dimmed `.caliper`.

### Workshop live feedback —「灯下看样 lights-down proofing」(1.13.0+)

Design spec: `docs/workshop-proofing-rev1.html`. Root problem it fixes: while adjusting workshop sliders the preview was invisible (mobile: 92dvh sheet + scrim left ~20px of canvas) or dimmed+blurred (desktop: full-viewport scrim). Governing principle: **the canvas is the first feedback surface — during printing, the darkroom yields to the paper.** Mechanisms ship one PR at a time:

- **S0 · bench drag low-res rendering** (`app.js` — `benchDrag` module right after `requestRender`). Delegated `pointerdown` binding on `#bench-stage` for every `input[type="range"]` (future harness knobs inherit for free); `doRender` passes `customScale: 0.2` while `benchDrag.live`, settle timer fires one full-res render 120ms after `pointerup`/`pointercancel` (both bound on `window` — implicit pointer capture retargets events to the input but they still bubble). **Canvas CSS box lock**: `#preview-canvas` is CSS-sized by its intrinsic pixel dims (`width:auto` + `max-width`), so dropping to 0.2-scale would visibly shrink the on-screen canvas — `lockCanvasBox()` freezes the CSS box at drag start, `doRender`'s finally unlocks once a full-res frame painted. `object-fit:contain` absorbs aspect changes mid-drag (padding slider). The rendering dot (`#preview-loading`) is suppressed during drags — 60fps flicker is noise. **Pause-to-crisp** (1.16.1): 200ms of stillness while still holding (`BENCH_PAUSE_CRISP_MS`) renders one full-res frame mid-drag (`benchDrag.crisp` flag) — the moment you hold still is exactly when you're judging the result; the next movement drops back to low-res.
- **A · scrim retreat** (`styles.css`). Desktop `.workshop-overlay::before` is now `background: transparent` (click-outside-to-close unchanged — pointer handling lives on the overlay div, not the ::before); the phone media block re-adds the dim behind the bottom sheet. On ≥1100px viewports `body[data-workshop-open="true"] .pane-canvas { padding-right: 372px }` re-centers the photo in the space the drawer leaves (440px drawer − 80px filmstrip ≈ 360px overlap; 701–1099px skips the shift — the drawer dominates the pane there). `openWorkshop`/`closeWorkshop` own the body flag.
- **B ·「关灯看样」drag choreography** (`benchLive` IIFE in `app.js`, 1.14.0). 60ms after pointerdown (`ENGAGE_MS` — a bare track-click doesn't flash the room) the workshop enters live state: `#workshop[data-live]` + `body[data-bench-live]`, the active row gets `.bench-live-row`, everything else disappears entirely (1.16.2 user ruling: `.bench-dim` opacity 0, no residual ghost; the bench SURFACE — background/border/shadow — clears on both desktop and mobile, and the active row itself floats SEMI-transparent at 0.55 with a translucent box-shadow-spread backdrop so the photo shows through even beneath it). The row's ancestor chain stays at opacity 1 but sheds its own surfaces via `.bench-live-path` (instrument-card graphite, frame accent bars, panel chrome would otherwise be the one thing still veiling the photo — background/border/box-shadow transparent, ::before/::after opacity 0). **The dim is structural**: opacity does NOT punch through a faded ancestor (a child at opacity:1 inside an opacity:.05 parent stays dark — trap hit live while building the design-doc mock), so `applyDim` walks the active row's ancestor chain up to `#workshop` and dims only the *siblings* at each level via `.bench-dim` + `.bench-dim-t` (the `-t` class carries the transition and outlives the state class so the fade-back animates). The "row" is found generically — walk up while the container holds exactly one range input, capped at the tool panel — so single-knob cards light the whole card (deliberate: card tag gives context) and future knobs need no registration. Release → settle render → 240ms beat → restore. Keyboard arrows enter the same state (600ms exit window). Mobile adds only: scrim/drag-pill hidden, topbar/lookbar dim (surface clearing + row backdrop are base rules since 1.16.2). The slider must never move under the finger — all choreography is opacity/background only, never transform/size.
- **C · HUD** (1.14.0; region hairline removed 1.16.3). `#bench-hud` reuses the `.compose-hud` class family (seal-hud precedent; position:fixed z-99 so it never dims with the workshop) and simply mirrors the active row's label + `<id>-val` readout text — zero extra i18n or formatting. The 1.14.0 amber region hairline (`#bench-region`, SLIDER_REGION/REGION_RESOLVERS maps, discrete-control flash, `CR.getLastPreviewLayout`) was **removed in 1.16.3 by user ruling**: a dashed box drawn on the very picture being judged is noise, not signal — for whole-canvas knobs (bg/margin) it outlined everything, and for localized knobs it overlaid exactly the area being watched. If a future feature needs on-canvas rect annotation, re-expose the last preview layout from `renderPreview` and map layout rects → CSS px against the canvas's bounding box (the removed implementation is in git history at 1.16.2).

- **D ·「试印对比 ◐」** (`benchProof` IIFE in `app.js`, 1.15.0). `openWorkshop` pins a proof — `createImageBitmap(els.canvas)` of the render at bench-open. Hold `#bench-proof-btn` (bench identity row) → the proof paints straight onto the canvas contain-fit (same pixel-replacement shape as `enterPeek`, no DOM overlay) with the HUD labeled 开台样张; release → `requestRender()`. Double-click re-pins (status toast `status.proofRepinned`); the `phototools:photo-switched` custom event re-pins automatically while the bench is open (comparing against another photo's render is meaningless — Q4 ruling, adjusted). Semantics vs peek: **peek = the raw photo, proof = the framed render before this bench session** — both live. Also fixed here: `isInteractiveTarget` in `wireKeyboardPeek` now lets `input[type=range]` through, so space-peek works right after dragging a knob (the knob keeps focus, which used to dead-end the gesture exactly when comparison matters).
- **E ·「◫ 试印条 test strip」** (`TESTSTRIP` IIFE in `app.js`, 1.16.0). Sweeps the last-touched knob (`benchDrag.lastInput`) across its full range in 4 evenly-spaced stops and composites them as vertical bands of ONE composition on the preview canvas — band labels at the TOP (on phones the lit row covers the canvas's lower half), the walked-in band outlined solid amber. **Ladder trick**: values are built by driving the real slider (`el.value = v` + dispatch `input`) with `benchStrip.busy` raised — the knob's own handler writes cfg and formats the readout, so there's no per-knob field map and band labels always match the row's formatting; `snapshotCfgForRender` is captured per stop, then 4 sequential `CR.renderPreview` calls at `customScale 0.25` into detached canvases. **Race guard** (bug hit during build): the ladder's dispatches call `requestRender()`, and a late full-res frame would overwrite the composited bands, whose self-heal guard in `doRender` would then kill the strip — so `requestRender` early-returns while `benchStrip.busy`, the pending drag-settle timer is cleared on entry, and `busy` stays raised until the composite lands. Adoption: canvas click → band index from clientX → `input.value = values[i]` + dispatch (cfg write + render + a brief lights-down beat as the receipt). Exits: ◫ toggle, Esc (capture-phase listener beats the workshop's close handler), grabbing any knob, photo switch, or any external render (self-heal in `doRender`). Strip mode reuses the live choreography via `benchLive.stripEnter` with `body[data-bench-strip]` yielding the workshop's pointer-events to the canvas (identity row + lit row re-enable theirs).
...
**`frameless`** (1.12.0+) — global "the canvas IS the photo" switch. When true: canvas aspect = the photo's effective (post-rotation-safe-area × crop) aspect, all four padding edges hard-overridden to 0, radius 0, and compose skips shadow / `clipPath` / `decorate` / caption / topTemplate badge — while `frame.fx` and the seal still run (they're the point of the mode; the halftone use-case is "export just the screened image"). Engine implementation is one shared helper, `R.applyFramelessLayoutOpts(layoutOpts, metaW, metaH)` (called from `buildLayoutAndCaption` in clientRender.js + the worker.js mirror): it synthesizes a `"W:H"` token for the existing `resolveAspectPreset` dynamic path (short-edge-1440 × quality semantics unchanged; ratio clamped to the parser's [0.1, 10] so extreme panos letterbox instead of throwing) and zeroes every asymmetric-layout opt. UI: Caliper's top toggle (`#frameless-toggle`, deliberately OUTSIDE `.caliper` so the `.is-frameless` body-class dimming of the geometry + caption zones can never disable the toggle itself). Semantics: **survives frame switches** (it's "what product do I want", like `aspect` — not a frame-tuned param like bg\*/shadow\*); in LOOK_KEYS (additive v:1; export projection ships it truthy-only like `captionForceOverlay`); Compose mode's projection deliberately forces `frameless: false` because Compose is the instrument for editing the very margins frameless hides. The `riso-halftone` factory seed sets `frameless: true`; every other seed explicitly sets `false` so applying one exits the mode (design ruling: only the halftone look is frameless-by-default).

**`htInk` / `htPaper` / `htCell` / `htPx` / `htAngle` / `htShape` / `htTone`** (1.12.0+) — halftone frame's seven screen knobs, declared via the `frame.cfg` harness (see the frames table + "Frame `fx` hook"). `htInk`/`htPaper` introduced the harness's `color` kind: schema carries a `swatches: [...]` array, UI renders a preset-swatch row + free `<input type="color">`, cfg stores a `'#rrggbb'` string (`null` = frame default; malformed values fall back at `resolveRenderParams`). `htPaper` also takes over `bg.color` (instax tintColors pattern) so photo highlights that drop out to bare paper merge seamlessly into the margin.
...
When debugging "the new cfg field doesn't seem to do anything," historically #3 / #4 were the easiest footguns (the 0.19 round shipped with both projections missing radiusOverride / captionForceOverlay; 1.10.4 caught applyFrameToAll missing 7 fields between 0.22 and 1.1). The 1.10.5 LOOK_KEYS-driven refactor of `doRender` / `buildConfigForFile` / `applyFrameToAll` retires that whole class of bug for any field that lives on LOOK_KEYS — drift surfaces remaining are #5 / #6 / #7 / #8 / #9 / #10 / #11, which are still hand-maintained. (1.12.0 caught the LAST hand-maintained projection: Compose mode's `composeProjection` claimed to mirror doRender but had silently dropped every harness knob added after 1.1 — gal\* / f35\* / instax\* / slide\* were invisible in Compose previews. It now iterates LOOK_KEYS too, with one deliberate override: `frameless: false`, because Compose edits the very margins frameless hides.)
...
**Bumping the cache**: when you change shell behavior (new precache asset, change to render pipeline that breaks compat with old cached files), bump `CACHE_VERSION` in `service-worker.js`. The activate handler purges caches that don't match. **Pre-commit check (twice-missed trap)**: if `git diff --name-only` for the commit touches ANY precached file (index.html, any `public/*.js`/`*.css`, frames/*, vendored libs), the same commit must bump `CACHE_VERSION` — the 1.13.0–1.16.0 run shipped four shell-changing commits without a bump, so previously-installed SWs served stale/mixed assets during local acceptance and the user re-reported an already-fixed bug.
...
- **Content-anchored CSS-block removal can silently take a SHARED rule that physically sits inside the range.** When deleting a feature's CSS by splicing from one comment-anchor to another (e.g. the 1.10.7 crop-modal removal spliced "Crop modal — darkroom enlarger" → "EXIF GPS row"), a base/shared rule authored inside that block disappears as collateral. The crop-modal section happened to contain the shared `.modal-x` base rule (the circular ✕ used by *every* modal head), so removing the crop CSS unstyled all 6 modal close buttons across the app — only surfaced weeks later when a user opened the changelog modal and saw a default browser box. The splice's region-safety check confirmed the range *contained* crop selectors but didn't check it *also* contained selectors used elsewhere. **Before splicing out a feature's CSS block, grep the to-be-removed range for any selector referenced outside that feature** (shared primitives like `.modal-x`, `.btn`, `.chip`, base form rules). If found, hoist it out first. Contextual overrides that assume a base (`.export-modal-head .modal-x { width: 36px }`) are a tell that a base rule exists somewhere and must survive.
...
- **`R.computeLayout` returns FLAT fg keys (`fgLeft`/`fgTop`/`fgW`/`fgH`), NOT a nested `fg: { x, y, w, h }` object.** I assumed the latter when writing the Compose mode and accessed `layout.fg.w` everywhere — every access returned `undefined`, `.w` on undefined threw `TypeError: Cannot read properties of undefined (reading 'w')`, the outer try/catch swallowed it, and the user saw "everything is broken" with no visible error. **Diagnostic signature**: dialog opens, leader shows correct source dims (so the bitmap loaded), but bench inputs are empty, canvas appears mis-sized, and the only console log is `[compose render] TypeError: Cannot read properties of undefined (reading 'w')`. **Fix**: when calling `R.computeLayout`, destructure with `const { canvas, fgLeft, fgTop, fgW, fgH } = layout`. **Lesson**: before destructuring a returned object, grep for the actual `return {...}` statement in the implementation — assumptions about API shape are silent footguns, especially when wrapped in try/catch.
- **Project's global `input[type="number"]` rule will beat your `.my-class` rules on specificity (0,1,1 vs 0,1,0).** The global rule in `public/styles.css` sets `font: 400 13px/1 var(--f-ui)` (Hanken Grotesk) and `padding: 8px 10px` — both win over a plain class selector. Surface symptom: input that should be JetBrains Mono renders Hanken Grotesk; that you set `padding: 0` on but `getComputedStyle` reports `12px`; visible content area is half what you expected and 4-digit numbers like `"4160"` clip to `"416("`. **Fix**: use `input.my-class` (matches the global's 0,1,1 specificity; later cascade order wins) or `.parent .my-class` (0,2,0). **Diagnostic**: open DevTools or run `getComputedStyle(el).fontFamily` — if it returns the project's UI font instead of your scoped one, you have a specificity miss. **The range-slider variant of the same trap shipped blind for multiple versions**: `input[type="range"]::-webkit-slider-thumb` (0,1,2 with the attribute + pseudo counted) beats `.torn-slider::-webkit-slider-thumb` (0,1,1), so several instrument cards' custom thumb styles never rendered — every card showed the global red thumb and nobody noticed because red-on-dark still looked "designed". Caught during the 1.12.1 reskin; the unified slider rules are prefixed `.workshop--bench .x-slider` (0,2,x) to out-rank the global. When styling a form control inside a scoped component, start the selector with the component root.
- **CSS `ch` unit is calibrated to the '0' glyph, NOT the widest digit.** `width: 4ch` in JetBrains Mono gives ~28px even though `"4160"` rendered with tabular-nums actually needs ~38px. For monospace inputs that show variable-width digit strings, use explicit pixel widths matched to the widest expected value (e.g. `width: 56px` for 4 digits) instead of `Nch`. Otherwise the last char gets clipped.

- **Procedural frame effects must sample and seed in SCALE-FREE space, or different render scales show different pictures.** Found via user acceptance of 1.13-1.16 ("拖拽时的预览效果和最终有出入"): torn's tear silhouette, film-mf's foxing spots and slide-mount's unevenness patches all derived their rng from CANVAS-PX geometry (`hashGeom(fgLeft, fgTop, fgW, fgH)`) and/or sampled at canvas-px spacing — so the 0.2× lights-down drag frames rendered a *different* tear/spot pattern than the 0.5× rest frame, visibly jumping on release (and Compose's 0.2× drag had the same latent bug since 1.1). Two rules for any future procedural `decorate`/`clipPath`/`fx` work: (a) **seed from scale-free identity** — position as a canvas fraction (×32 buckets), cfg knob values, EXIF — never from pixel dims; (b) **hash per candidate/vertex INDEX (`h01(seed, channel, i)`), never consume a sequential rng stream through rejection sampling or variable-count loops** — one borderline rejection flipping between scales (rounded layout px make ratios differ by ~1e-3) shifts every subsequent draw and re-rolls the whole field. Reference implementations: `torn.js` (indexed edge vertices, base-1440 sampling), `film-mf.js` (indexed foxing candidates), `slide-mount.js` (indexed patches). Also scale-sensitive: `createPattern` tiles drawn at canvas resolution — compensate with `pattern.setTransform` (slide-mount pebble does; drawGrain compensates below PREVIEW_SCALE since 1.16.1). Two more sub-traps from the same hunt (halftone, worst offender at ~40% pixel flips): (c) **never clamp/floor base-unit-derived math per-scale** — halftone's `chunk = Math.max(1, px·scale)` silently switched the screen to a different working resolution whenever `px·scale < 1`; (d) **binary/base-res output needs smooth downscaling when the canvas can't resolve it** — nearest-neighbor below Nyquist shreds a lattice into aliasing debris; smoothing shows the honest local average so the settle frame only sharpens.
- **Vendored UMD tails can assume `window` and silently kill the whole worker pool.** piexif.js ended with `window.piexif = that` — in a worker there is no `window`, so `importScripts('vendor/piexif.js')` threw `ReferenceError` on line 1 of pool startup, the `ready` message never arrived, and exporter.js's fallback quietly ran EVERY batch export on the main thread — from the project's first commit until 1.12.0. Nothing user-visible failed (the fallback works, just serially + jankily), which is exactly why it survived so long. Fixed by patching the vendored tail to `(typeof window !== 'undefined' ? window : self).piexif = that`. **Lessons**: (a) when vendoring a UMD lib that workers will `importScripts`, check its global-attach tail for bare `window`; (b) a silent fallback path needs at least one loud probe — verify a worker actually reaches `ready` before trusting "batch export works" as evidence that workers work.
- **smoke.html can be broken-silent for months if nobody re-runs it after the change class it exists to catch.** The 0.22 frame cull renamed frames but smoke's fixtures kept the old names AND indexed `FRAMES[name]` directly (aliases only resolve via `resolveFrame`) — so the page threw before rendering fixture 1 and sat at "running…" forever. Every session between 0.22 and 1.12.0 that skipped smoke (per rule 3's "only for pipeline changes") never noticed. **Lessons**: (a) fixture/test code must resolve frames through `resolveFrame`, never direct `FRAMES[...]` indexing; (b) renaming/retiring a frame IS a pipeline change — run smoke as part of it; (c) a stuck "running…" with a console `TypeError` means the harness itself is broken, not the pipeline — check the console before reading the diff numbers. Related trap discovered while debugging: `frosted-dark` / `white` are NOT `FRAME_ALIASES` entries — they're live DOUBLE-REGISTRATIONS (frosted-noir.js / gallery-white.js call `registerFrame` twice with the same def), which is also why the localhost `[frame-cfg] duplicate key` console warns fire for bg\*/gal\* — the sanity check iterates `Object.values(FRAMES)` and sees those defs twice. That warn noise is benign and pre-existing; don't burn a round "fixing" it by deleting the second registration (old share-codes use those names).
