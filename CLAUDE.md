...
**Mobile parity (1.1.2)**: the mobile Compose dialog is a **parallel UI**, not a CSS-reflow of the desktop bench. Same `<dialog id="compose-modal">` but new markup elements (`<nav id="compose-mode-pills">` / `<div id="compose-mobile-pad-strip">` / `<div id="compose-mobile-actions">` / `<div id="compose-numeric-sheet">`) appear only on `max-width: 700px`, while the desktop `<footer class="compose-bench">` hides via `display: none` in the same media query. Both surfaces share the same JS state (`COMPOSE.cfg` / `setFocus` / `applyAspectChip` / `setRotationDeg`) — the new mobile bindings (`bindMobileModePills` / `bindMobilePadStepper` / `bindNumericSheet`) just route into the same setters.

Key mobile UX choices:
...
- **Aspect chips horizontal-scroll** instead of wrapping (`overflow-x: auto` + `scroll-snap-type: x mandatory` + right-edge fade mask). 8 chips fit on one line; user swipes to see them all.
- **Padding mode swaps capsule handles for stepper segments**. On-photo `.compose-pad-handle` is `opacity: 0 + pointer-events: none` under mobile, replaced by 4 segments of `[-] [value] [+]` in the bottom strip. Single tap = ±5px; long-press ≥280ms triggers a 60ms-interval repeat for fast sweeps. The pad-handle was 14×40 px in desktop, completely un-grabbable on touch — splitting into stepper buttons gives WCAG-conformant 28×36 px targets each.
- **HUD bubble offsets away from finger** on mobile (touchX+30, touchY-60), with auto-flip when too close to viewport edges. `isMobileViewport()` matchMedia drives the branch.
- **Photo gets ≥ 70% of viewport** vs ~30% in the v1.1.0 mobile layout — measured on 390×844: chrome budget is 48 (topbar) + 54 (context strip) + 60 (mobile actions) + safe-area-inset ≈ 196 px, photo gets ~648 px.
