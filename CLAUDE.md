...
- **C · HUD + region hairline** (1.14.0). `#bench-hud` reuses the `.compose-hud` class family (seal-hud precedent; position:fixed z-99 so it never dims with the workshop) and simply mirrors the active row's label + `<id>-val` readout text — zero extra i18n or formatting. `#bench-region` (inside `.canvas-stage`, class-toggled `.is-on` — never the `hidden` attr) outlines the region a knob affects: `SLIDER_REGION` maps knob id → region kind, `REGION_RESOLVERS` maps kind → canvas-space rect from `CR.getLastPreviewLayout()` (exposed by `renderPreview` in 1.14.0), scaled into CSS px against the canvas's bounding box. Rotated caption zones swap width/height back to the on-canvas box. Unmapped sliders get HUD only — the region is progressive enhancement. Discrete controls (`CONTROL_REGION` list: topTemplate seg, overlay toggle, show-fields, frameless) flash the region for 900ms on click, no dim (Q3 ruling in the design doc).
