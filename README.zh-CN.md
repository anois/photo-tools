...
裁剪、旋转、四边距合并进一个全屏直接拖拽的界面 —— 从 lookbar 的 **构图** 入口进入（与 LOOK + 4 个 lookchip 并列的第 6 个独立块）。交互语言用空间分层区分：

- **照片内部** = 裁剪。拖角括号收紧构图。
- **画框边缘** = 边距（每边独立）。推胶囊条改变画框纸边形状。
- **顶部圆环** = 旋转。默认 90° 吸附；按住 `Shift` 进入 1° 自由档。
- **拖动照片内部** = 平移裁剪框中心。

被裁掉的部分**继续保留在画面上**，以 30% 透明度作为"幽灵层"显示在渲染预览之后 —— 你随时看到什么被切掉。每个相框声明了它的 `minPadding` 推荐值（比如 film-35 顶部 ≥ 70px、底部 ≥ 90px 给齿孔留位置），低于阈值时该边出现淡红警示带，**只提醒不强制**。底部"工作台"长驻显示当前数值（`W × H`、`T / R / B / L`、角度），每个数字都是可输入框，方便精确构图。
...
| **构图** | 拼贴 2 / 3 / 4 格 · 90° 旋转 · 自由裁剪（渲染时应用，源图不动）· GPS 自动解析 + 手填 + 地图选点（Leaflet + 高德地图，GCJ-02 ↔ WGS-84 校正） |
