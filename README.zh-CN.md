...
产品模型是**引擎 + 社区风格库**，不是"固定相框 + 几个滑块"。当种子不太对你这张照片的味，工作台展开就是引擎所有暴露出来的旋钮：圆角、水印嵌入图片 + 离底高度、撕纸抖动 / 密度 / 暗边、复古印品老化程度、顶部标记 picker、自定义背景图、阴影三连、签名水印。每一个改动都可以存成新 LOOK，可以分享，也可以一键应用到整条胶卷。
...
裁剪、旋转、四边距合并进一个全屏暗房工作台 —— 从 lookbar 的 **构图** 入口进入（与 LOOK + 4 个 lookchip 并列的第 6 个独立块）。**三种工具严格互斥**，点击底部工作台对应模块激活该工具，再在照片上操作：

- **裁剪** —— 照片四角的琥珀色角括号收紧裁剪窗口；拖照片内部平移裁剪框。底部悬浮一条预设比例 chip 条（自由 / 当前画幅 / 1:1 / 3:4 / 4:3 / 9:16 / 16:9 / **自定义 W:H**），跟外部画幅 picker 同一套输入 UX。
- **边距** —— 四个胶囊推条贴在照片与画框之间。每边独立调整（`paddingTop / Right / Bottom / Left`）。
- **旋转** —— 照片下方滑入一条 360° 拖拽条。范围 −180° 到 +180°，步长 0.5°，配 ±90° 快捷按钮 + 归零。引擎本来就支持任意角度，这次把能力暴露到 UI。

裁剪与旋转期间被切掉的部分**继续保留在画面上**，以 30% 透明度作为"幽灵层"显示在渲染预览之后 —— 你随时看到什么被切掉。每个相框声明了它的 `minPadding` 推荐值（比如 film-35 顶部 ≥ 70px、底部 ≥ 90px 给齿孔留位置），低于阈值时该边出现淡红警示带，**只提醒不强制**。底部工作台长驻显示当前数值（`W × H`、`T / R / B / L`、角度），每个数字都是可输入框，方便精确构图。拖动期间渲染降到 1/6 像素量保证 60fps 流畅，松手 120ms 后自动恢复高清。
